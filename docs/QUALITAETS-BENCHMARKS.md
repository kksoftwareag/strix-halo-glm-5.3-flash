...
| Quant-Wechsel (Neustart des Servers) | ca. 30 s | eigene Messung |
...
| AIME 2025 (lm-eval) | 30 Aufgaben | keine | 3 bis 5 h bei max | schlecht: nur 30 Aufgaben |
...
uv run python bench/quality/tbench.py --tier full --quant UD-Q2_K_XL
```

`tbench.py` startet den Server mit dem Preset `tkmtp-agent` (UD-IQ2_XXS, 131072 Kontext, ein Slot,
MTP Draft-Tiefe 2, kleiner Prompt-Cache), wartet auf `/health`, übergibt Endpunkt, Kontextlänge und die
...
| Qwen3.8-27B | UD-IQ2_XXS | llama.cpp/rocm | 16/20 | 19/20 | 15 h |
...
| Qwen3.6-35B-A3B | UD-IQ2_XXS | llama.cpp/rocm | 11/20 | 11/20 | 15 h |
...
| `--concurrency 4` mit UD-Q2_K_XL | 20 Aufgaben parallel in 4 Slots | 8 bis 12 h | pass@1, etwa 30 % schneller |
...
./serve.sh --preset multiuser-4 --np 8 --reasoning low   # 8 Slots, MTP aus
